...
def imgEncode(msg):
    ...
    for x in range(512):
        ...
        try:
            bit = msg.pop()             #odd pixels = 1, even pixels = 0
            if brightness(x, y, lenna)%2==0 and bit==1: 
                lenna[y, x, :] += 1
            elif brightness(x, y, lenna)%2==1 and bit==0:
                lenna[y, x, :] += 1
        except:
            continue
        
    cv2.imwrite('encoded.png', lenna)
    print("Done")

def imgDecode():
    msg = ""
    lenna = cv2.imread("encoded.png")

    for x in range(512):
        y = drawSinforImg(x)
        try:           #odd pixels = 1, even pixels = 0
            if brightness(x, y, lenna)%2==0:
                msg += 0
            elif brightness(x, y, lenna)%2==1:
                msg += 1
        except:
            continue
